...
START_AFTER_LOGIN = 'soft'   # 'require' (login olmadan başlamaz), 'soft' (X sn bekle, sonra başla), 'none' (hemen başla)
LOGIN_GATE_WAIT = 90         # 'soft'/'require' için login'e toplam bekleme (sn)
ABORT_ON_2FA = True          # 2FA ekrani algılanırsa işlemi hemen kes ve raporla
# =========================
...
class WorkerSignals(QObject):
    finished=Signal(); status=Signal(str); progress=Signal(int,int,str); logline=Signal(str)
...
class RotatorWorker(threading.Thread):
    ...
    # ----- Login Helpers (no page trashing) -----
    ...
    def _google_sign_in_blocking(self) -> bool:
        """Login'i AYRI bir sekmede yapar, ana sekmeyi bozmaz; sınırlı bekleme.
        2FA algılanırsa (ABORT_ON_2FA=True) hemen abort eder.
        Stop talebi gelirse anında keser.
        """
        d=self.controller.chrome
        if not d: 
        ...
        # Challenge / 2FA / reCAPTCHA bekleme (sınırlı)
        start=time.time(); last_note=0
        success=False
        ...
        # Login sekmesini kapat ve ana pencereye dön
        try: d.close()
        except Exception: pass
        try: d.switch_to.window(main)
        except Exception: pass
    ...
    def run(self):
        try:
                ...
                while time.time()-t0 < LOGIN_GATE_WAIT:
                    ...
                    if LOGIN_MODE in ('auto','force') and self.login_email and self.login_password:
                        ...
                        if self.abort_reason == '2FA_DETECTED':
                            self.signals.logline.emit("2FA algılandı → işlem sonlandırılıyor (START_AFTER_LOGIN fark etmeksizin).")
                            break
                        if login_ok: break
                    time.sleep(1)
            ...
            for url, mins in self.urls:
                ...
                total=max(1,int(mins))*60; self.signals.status.emit(f"Açılıyor: {url} — {mins} dk")
                ...
                while not self._stop.is_set():
                    ...
                    if elapsed % 5 == 0: self.signals.progress.emit(elapsed,total,url)
                    time.sleep(1)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        top=QHBoxLayout(); self.lbl_status=QLabel("Hazır."); self.progress=QProgressBar(); self.progress.setRange(0,100); self.progress.setValue(0)
        top.addWidget(self.lbl_status,3); top.addWidget(self.progress,2); top.addWidget(QLabel("Grup:"))
        self.in_active_group=QLineEdit(); self.in_active_group.setPlaceholderText("örn. Gece"); top.addWidget(self.in_active_group); root.addLayout(top)
    ...
    def delete_selected(self):
        for it in self.listw.selectedItems():
            log_append(self.log,f"URL silindi: {(it.data(Qt.UserRole) or ('',0))[0]}"); self.listw.takeItem(self.listw.row(it))
        self._save_click()
    ...
    def on_progress(self, elapsed:int, total:int, url:str):
        pct=int(max(0,min(100,(elapsed/total)*100))); self.progress.setValue(pct)
        mm_e,ss_e=divmod(elapsed,60); mm_t,ss_t=divmod(total,60)
        self.set_status(f"{normalize_youtube_url(url)} — {mm_e:02d}:{ss_e:02d} / {mm_t:02d}:{ss_t:02d}  ({pct}%)")
